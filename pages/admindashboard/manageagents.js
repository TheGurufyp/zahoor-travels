...
function Manageagents({ responseFromServer }) {
  ...
  const clickedYes = (user_id) => {
    axios
      .post(
        `${process.env.NEXT_PUBLIC_HOST}/deleteagent`,
        {
          user_id: user_id,
        },
        { headers: { token: token } }
  ...
  const submitFile = (e, username) => {
    ...
    promise
      .then((data) => {
        ...
        axios
          .post(
            `${process.env.NEXT_PUBLIC_HOST}/uploadmutamers`,
            {
              mutamers: data,
              username: username,
            },
      ...
      <Box mx="10px" mb="30px">
        ...
        >
          <Table variant="striped" colorScheme="blue">
            <TableCaption mt="0px" fontSize={"2rem"} placement={"top"}>
              Manage Agents
            </TableCaption>
            {agents.length <= 0 ? (
              <TableCaption mt="0px" fontSize={"1rem"} placement={"bottom"}>
                No Agents Found
              </TableCaption>
            ) : (
              <>
                      ...
                      <Tr key={agent._id}>
                        ...
                        <Td isNumeric>
                          ...
                          <Input
                            ...
                            type="file"
                            onInput={(e) => {
                              submitFile(e, agent.username);
                            }}
                          />
                          <EditAgentModal user_id={agent._id}>
                            <Button mr="6px" size={"sm"} colorScheme={"yellow"}>
                              {" "}
                              Edit
                            </Button>
                          </EditAgentModal>
