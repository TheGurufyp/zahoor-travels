...
function Navbar() {
  ...
  const handleLogout = () => {
    ...
    if (user?.isAdmin) {
      axios
        .post(
          `${process.env.NEXT_PUBLIC_HOST}/adminlogout`,
          {},
          { headers: { token: token } }
        )
      ...
      axios
        .post(
          `${process.env.NEXT_PUBLIC_HOST}/userlogout`,
          {},
          { headers: { token: token } }
        )
        .then(function (response) {
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Box>
            {" "}
            <HiChevronDown color="white" strokeWidth={"1px"} size={"1rem"} />
          </Box>
        </Flex>
      </Flex>
    </>
