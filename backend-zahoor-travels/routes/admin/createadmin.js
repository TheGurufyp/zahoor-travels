...
router.post("/", [adminAlreadyExists,authAdmin],async (req, res) => {

    if (req.alreadyExists == false) { // create new admin
        ...
        if (req.body.password) // password is defined in body
        {
            ...
            newAdmin.save().then(() => {
                res.send({ success: true, payload: "New Admin Created Successfully" });
            }).catch((error) => {
